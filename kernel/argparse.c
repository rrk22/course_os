...
static void argparse_parse(char *cmdline)
{
	...
	while (token != NULL)
	{
		...
		if (os_strcmp("-load", token) == 0)
		{
			...
			//there is a definition at the top of scheduler c which
			//converts pcb into task 
			sched_task *task = sched_create_task_from_process(proc, 0);
			//the scheduler does need to start. should it happen here?
			sched_add_task(task);
			sched_start();
			//do we need task p or this second line here?
